...
export default function AllClientsPage() {
  ...
  // these are pagination states
    ...
    const [data, setData] = useState<RawClient[]>([])  
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await safeGet<{ data: RawClient[]; meta?: { total?: number } }>(
          `/clients?${queryString}`
        );
        if (!mounted) return;
        ...
        if (res === null) {
          ...
          const mapped: Item[] = (res.data ?? []).map((raw) => ({
            id: String(raw.id ?? ""),
            name: raw.name ?? "",
            address: raw.address ?? "",
            personInCharge: raw.contact_person ?? "",
            phone: raw.phone ?? "",
