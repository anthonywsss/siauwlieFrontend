...
export default function AllItemsClient() {
  ...
  const [assetFilter, setAssetFilter] = useState<string>("all"); // Changed to string
    ...
    (async () => {
      try {
        ...
        if (err?.response?.status === 401) {
          signOut();
          try { router.push("/auth/sign-in"); } catch {}
          return;
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const types: AssetType[] = res?.data?.data ?? res?.data ?? [];
        if (!mounted) return;
  ...
  function handleEditOpen(raw: RawAsset) {
    setEditingRaw(raw);
  }
  ...
  const assetTypeMap = useMemo(() => {
    ...
    allTypes.forEach(t => { map[t.id] = t.name });
    return map;
  }, [allTypes]);
  ...
  const ASSET_TYPE_OPTIONS = [
    ...
    ...allTypes.map(t => ({ value: t.id.toString(), label: t.name }))
  ];
  ...
  const visibleItems = useMemo(() => {
    return data.filter((item) => {
      ...
      const queryMatches = !query || 
        ...
        item.current_client?.toString().includes(query);
      
      return statusMatches && assetMatches && queryMatches;
    });
  }, [data, statusFilter, assetFilter, query]);

  // Paginated items
  const paginatedItems = useMemo(() => {
    return visibleItems.slice((page - 1) * perPage, page * perPage);
  }, [visibleItems, page, perPage]);

  // History navigation
  const goToDetail = (assetId: string) => {
    router.push(`/unfinished-copy/${encodeURIComponent(assetId)}`);
  };
      ...
      <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-3 flex-wrap w-full md:w-auto">
            ...
            {/* Asset Type Filter dropdown - FIXED */}
        ...
        {/* RIGHT: add + total */}
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="text-sm text-gray-500 ml-auto md:ml-0">
            {loading ? "Loading..." : `${visibleItems.length} of ${total} items`}
          </div>
          <button 
      ...
      <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <form onSubmit={handleGotoSubmit} className="flex items-center gap-2">
            ...
            />
            <button type="submit" className="rounded border px-3 py-1">Go</button>
          </form>
        </div>
